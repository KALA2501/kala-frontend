...
const FaqViewPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistorial = async () => {
      try {
        const res = await axios.get(`${API_GATEWAY}/api/formularios/faq/historial/${pacienteId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setHistorial(res.data);
        setEvaluacionSeleccionada(res.data[0]);
      } catch (err) {
        console.error('❌ Error al obtener historial FAQ:', err);
        alert('No se encontró historial FAQ para este paciente.');
      }
    };
  ...
  return (
    <div className="flex min-h-screen bg-offWhite">
      ...
      <main className="flex-1 p-8 animate-fadeInBlur">
        <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-2xl border-t-8 border-purple" id="evaluacion-faq">
          <div className="h-56 rounded-t-2xl overflow-hidden relative">
            <img src={faqImg} alt="Cabecera FAQ" className="w-full h-full object-cover object-center" />
          ...
          <div className="p-8">
            <h1 className="text-3xl font-bold text-center text-purple mb-1">Evaluación FAQ</h1>
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-base">
              {evaluacionSeleccionada && campos.map(([key, label]) => (
                <li key={key}><strong>{label}:</strong> {booleanToYesNo(evaluacionSeleccionada[key])}</li>
              ))}
            </ul>
        ...
        <div className="max-w-4xl mx-auto mt-10">
          <h2 className="text-2xl font-semibold text-[#30028D] mb-4">Historial de Evaluaciones FAQ</h2>
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right text-sm">
                    <p><strong>Teléfono:</strong> {booleanToYesNo(ev.usarTelefono)}</p>
                    <p><strong>Emergencias:</strong> {booleanToYesNo(ev.responderEmergencias)}</p>
                  </div>
                </div>
              </div>
            ))}
